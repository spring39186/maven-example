...
public class FindbugsWarningsByExample1 {
	...
	public static void main(final String[] args) {
		...
		// WRONG
		try {
			FindbugsWarningsByExample1.bcImpossibleCastWRONG();
		} catch (final ClassCastException e) {
			System.out.println("   - ERROR:" + e.getMessage());
		}
		// CORRECT
		FindbugsWarningsByExample1.bcImpossibleCastCORRECT();

		System.out.println("Findbugs Sample 002 for BC_IMPOSSIBLE_DOWNCAST");
		// WRONG
		try {
			FindbugsWarningsByExample1.bcImpossibleDowncastWRONG();
		} catch (final ClassCastException e) {
			System.out.println("   - ERROR:" + e.getMessage());
		}
		// CORRECT
		FindbugsWarningsByExample1.bcImpossibleDowncastCORRECT();

		System.out.println("Findbugs Sample 003 for BC_IMPOSSIBLE_INSTANCEOF");
		// WRONG
		FindbugsWarningsByExample1.bcImpossibleInstanceOfWRONG();
		// CORRECT
		...
		// CORRECT
		FindbugsWarningsByExample1.esComparingStringsWithEqCORRECT();

		System.out.println("Findbugs Sample 007 for VA_FORMAT_STRING_ILLEGAL");
		// WRONG
		try {
			FindbugsWarningsByExample1.vaFormatStringIllegalWRONG();
		} catch (final UnknownFormatConversionException e) {
			System.out.println("   - ERROR:" + e.getMessage());
		}
		// CORRECT
		FindbugsWarningsByExample1.vaFormatStringIllegalCORRECT();

		System.out.println("Findbugs Sample 008 for RV_RETURN_VALUE_IGNORED");
		// WRONG
		FindbugsWarningsByExample1.rvReturnValueIgnoredWRONG();
		// CORRECT
		FindbugsWarningsByExample1.rvReturnValueIgnoredCORRECT();

		System.out.println("Findbugs Sample 009 for NP_ALWAYS_NULL");
		// WRONG
		try {
			FindbugsWarningsByExample1.npAlwaysNullWRONG();
		...
		// CORRECT
		FindbugsWarningsByExample1.qabQuestionableBooleanAssignmentCORRECT();
